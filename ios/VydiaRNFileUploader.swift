...
class VydiaRNFileUploader: RCTEventEmitter, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    func getFileInfo(_ path: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        do {
            guard let escapedPath = path.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else {
            ...
            if exists {
                ...
                let attributes = try FileManager.default.attributesOfItem(atPath: pathWithoutProtocol)
    ...
    private func guessMIMETypeFromFileName(fileName: String) -> String {
        guard let uti = UTTypeCreatePreferredIdentifierForTag(kUTTagClassFilenameExtension, fileName.pathExtension as CFString, nil)?.takeRetainedValue() else {
    ...
    private func copyAssetToFile(assetUrl: String, completionHandler: @escaping (String?, Error?) -> Void) {
        guard let url = URL(string: assetUrl) else {
            completionHandler(nil, NSError(domain: "RNUploader", code: 5, userInfo: [NSLocalizedDescriptionKey: "Invalid asset URL"]))
            return
        }
        
        let fetchResult = PHAsset.fetchAssets(withALAssetURLs: [url], options: nil)
        guard let asset = fetchResult.lastObject else {
            completionHandler(nil, NSError(domain: "RNUploader", code: 5, userInfo: [NSLocalizedDescriptionKey: "Asset could not be fetched. Are you missing permissions?"]))
            return
        }
        
        guard let assetResource = PHAssetResource.assetResources(for: asset).first else {
            completionHandler(nil, NSError(domain: "RNUploader", code: 5, userInfo: [NSLocalizedDescriptionKey: "Asset resource not found"]))
            return
        }
    ...
    @objc
    func startUpload(_ options: [String: Any], resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        ...
        if uploadType == "multipart" {
            ...
            guard let httpBody = self.createBody(withBoundary: uuidStr, path: fileURI, parameters: parameters, fieldName: fieldName) else {
            ...
            uploadTask = self.getSession(appGroup: appGroup).uploadTask(withStreamedRequest: request)
        } else {
            ...
            uploadTask = self.getSession(appGroup: appGroup).uploadTask(with: request, fromFile: fileURL)
        }
    ...
    @objc
    func cancelUpload(_ cancelUploadId: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    ...
    private func createBody(withBoundary boundary: String, path: String, parameters: [String: Any], fieldName: String) -> Data? {
        var httpBody = Data()
    ...
    private func getSession(appGroup: String?) -> URLSession {
        ...
        let sessionConfiguration = URLSessionConfiguration.background(withIdentifier: VydiaRNFileUploader.BackgroundSessionId)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didSendBodyData bytesSent: Int64, totalBytesSent: Int64, totalBytesExpectedToSend: Int64) {
        ...
        self.sendEvent(withName: "RNFileUploader-progress", body: [
